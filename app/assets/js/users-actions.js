$(document).ready(function() {
    ...
    .then(data => {
        ...
        data.forEach(user => {
            ...
            //                 <ion-icon name="trash-sharp" id="${id}" class="icon icon-delete" style="color:red;" data-toggle="modal" data-target="#exampleModal"></ion-icon>
            // `
            ...
            <td>${statusBadge}</td>
            <td>
                <ion-icon name="create-sharp" class="icon" style="color:#17a2b8;"></ion-icon>
                <ion-icon name="reader-sharp" class="icon" style="color:gray;"></ion-icon>
                <ion-icon name="trash-sharp" id="${user[0]}" class="icon icon-delete" style="color:red;" data-toggle="modal" data-target="#exampleModal"></ion-icon>
            </td>
            `;
...
input.addEventListener('click', function() {
        var id = this.id;
        console.log(id + ' hola');
        var deleteButton = document.querySelector('#user-delete-button');
        deleteButton.setAttribute('user-id', id);
    });
});

function userDelete() {
    var id = document.getElementById("user-delete-button").getAttribute("user-id");  
    console.log(id)
    fetch('/delete_user', {
        method: 'POST',
        body: id
    ...
    .then(data => {
        if (data.success) {
            myAlertMessage('User deleted successfully', 'success');
            
            window.location.href = 'http://127.0.0.1:5000/users';

        } else {
            myAlertMessage('User was not deleted', 'error');
            setTimeout(function() {
                alertDiv2.style.display = 'none';
            }, 1000);
        }
